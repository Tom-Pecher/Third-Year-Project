...
from utils.Dueling_NN import DuelingNN
...
class DDDQNAgent:
    def __init__(self, env,
        ...
        if wandb_on:
            wandb.init(project="DDDQN-Training", config={
        ...
        # Using DuelingNN instead of regular NN
        self.policy_net = DuelingNN(n_observations, n_actions).to(device)
        self.target_net = DuelingNN(n_observations, n_actions).to(device)
    ...
    def optimize(self) -> None:
        ...
        # Get current Q values using dueling network
        state_action_values = self.policy_net(state_batch).gather(1, action_batch)

        # Double DQN with dueling architecture
        next_state_values = torch.zeros(self.batch_size, device=device)
        with torch.no_grad():
            # Use policy net to select actions (Double DQN)
            next_actions = self.policy_net(non_final_next_states).max(1)[1].unsqueeze(1)
            # Use target net to evaluate actions (the dueling architecture handles the value/advantage separation)
    ...
    def train(self, num_episodes:int=100, sumo_gui=False) -> None:
        ...
        for episode in range(num_episodes + 1):
            ...
            while True:
                ...
                observation, reward, terminated, episode_info = self.env.step(action)
